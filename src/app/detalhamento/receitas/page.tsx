...
import { extrairReceitas, ItemDetalhado } from '@/services/detalhamentoService'
import { TrendingUp, ChevronDown, ChevronRight, Download } from 'lucide-react'
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip, BarChart, Bar, XAxis, YAxis, CartesianGrid } from 'recharts'
import * as XLSX from 'xlsx'

const CORES = ['#10b981', '#3b82f6', '#8b5cf6', '#f59e0b', '#ef4444', '#ec4899', '#06b6d4', '#84cc16']
...
export default function ReceitasPage() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const { itens, meses, totalGeral } = extrairReceitas(dadosPorMes)
  ...
  const exportarExcel = () => {
    ...
    XLSX.writeFile(wb, 'detalhamento_receitas_tophaus.xlsx')
  }
  ...
  const organizarParaExibicao = () => {
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      if (item.hierarquia === 1) {
        ...
        // Visível se o pai H1 específico está expandido
        visivel = pais.paiH1 !== null && expandidos.has(pais.paiH1)
      }
      else if (item.hierarquia === 3) {
        // Visível se o pai H1 E o pai H2 específicos estão expandidos
        ...
        <div className="px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-800">Detalhamento de Receitas</h1>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-5 text-white">
            <p className="text-emerald-100 text-sm font-medium">Total Acumulado</p>
            <p className="text-3xl font-bold mt-1">{formatarValor(totalGeral)}</p>
          </div>
          <div className="bg-white rounded-2xl p-5 border border-slate-200">
            <p className="text-slate-500 text-sm font-medium">Tipos de Receita</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Composição das Receitas</h3>
            ...
            <table className="w-full">
              <thead className="bg-slate-800 text-white sticky top-0">
                <tr>
                  ...
                  <th className="text-right p-4 min-w-[130px] bg-emerald-700 sticky right-[100px] z-10">ACUMULADO</th>
                  <th className="text-right p-4 min-w-[100px] bg-emerald-800 sticky right-0 z-10">% TOTAL</th>
                </tr>
              </thead>
              <tbody>
                {itensParaExibir.map(({ item, visivel, temFilhos }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`p-4 text-right font-semibold tabular-nums sticky right-[100px] z-10 ${item.hierarquia === 1 ? 'bg-emerald-100 text-emerald-800' : 'bg-emerald-50 text-emerald-700'}`}>
                        {formatarValor(item.acumulado)}
                      </td>
                      <td className={`p-4 text-right font-semibold sticky right-0 z-10 ${item.hierarquia === 1 ? 'bg-emerald-200 text-emerald-800' : 'bg-emerald-100 text-emerald-700'}`}>
                ...
                <tr className="bg-emerald-600 text-white font-bold">
                  <td className="p-4 sticky left-0 bg-emerald-600 z-10">TOTAL GERAL</td>
                  {meses.map(mes => {
                  ...
                  <td className="p-4 text-right sticky right-[100px] bg-emerald-700 z-10">{formatarValor(totalGeral)}</td>
                  <td className="p-4 text-right sticky right-0 bg-emerald-800 z-10">100%</td>
                </tr>
              </tbody>
            </table>
          </div>
